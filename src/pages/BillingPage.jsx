...
export default function BillingPage() {
  ...
  async function startCheckout() {
    ...
    try {
      ...
      if (data?.alreadyActive) {
        await load();
        return toast.success("الاشتراك فعّال بالفعل.");
      }

      const token = import.meta.env.VITE_PADDLE_CLIENT_TOKEN;
      if (!token) throw new Error("مفتاح Paddle غير موجود في إعدادات المشروع.");
      ...
      toast.error(error?.message || "تعذر فتح صفحة الدفع.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function openPortal() {
    ...
    try {
      ...
      if (error) throw error;
      if (!data?.url) throw new Error("لم يتم إرجاع رابط إدارة الفوترة.");
      window.location.href = data.url;
    } catch (error) {
      toast.error(error?.message || "تعذر فتح إدارة الفوترة.");
    } finally {
      setBusy(false);
    }
  }
  ...
  if (!menu) return <Page><div className="rounded-[26px] border border-white/10 bg-[#111] p-6"><h1 className="text-2xl font-black">لم يتم تفعيل مطعم لهذا الحساب بعد.</h1></div></Page>;

  return (
    <Page>
      <p className="text-xs font-black uppercase tracking-[0.15em] text-[#ff7a00]">CRTGO BILLING</p>
      <h1 className="mt-2 text-4xl font-black tracking-[-0.055em]">الفوترة</h1>
      ...
      <div className="mt-8 max-w-2xl rounded-[28px] border border-white/10 bg-[#111] p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.14em] text-white/30">الخطة</p>
            <h2 className="mt-2 text-2xl font-black">{plan?.name || "CRTGO Menu"}</h2>
            <p className="mt-2 text-sm font-bold text-white/40">{plan ? `${plan.monthly_price} ${plan.currency} / شهرياً` : "—"}</p>
          </div>
          <div className={`rounded-full px-3 py-2 text-xs font-black ${active ? "bg-emerald-500/15 text-emerald-300" : "bg-white/5 text-white/45"}`}>
            {statusLabel(subscription?.status)}
          </div>
        </div>

        {subscription?.current_period_end && (
          <p className="mt-5 text-xs font-bold text-white/35">نهاية الفترة الحالية: {new Date(subscription.current_period_end).toLocaleDateString("ar")}</p>
        )}
        ...
        <button onClick={active ? openPortal : startCheckout} disabled={busy} className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-[#ff7a00] px-5 py-3 text-sm font-black text-black disabled:opacity-60">
          {busy ? <Loader2 size={17} className="animate-spin" /> : active ? <ExternalLink size={17} /> : <CreditCard size={17} />}
          {active ? "إدارة الاشتراك" : "بدء الاشتراك"}
        </button>
      </div>
    </Page>
  );
